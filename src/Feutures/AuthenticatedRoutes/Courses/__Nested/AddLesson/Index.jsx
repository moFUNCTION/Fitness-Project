...
export default function Index() {
  ...
  const createLessoneHelper = useCallback(async (data) => {
  ...
  const verfifyUpload = async (signedURL) => {
    const req = await axiosInstance.head(signedURL, {
      headers: {
        Authorization: `Bearer ${user.data.token}`,
        "Tus-Resumable": "1.0.0",
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await createLessoneHelper(DataSend);

      toast({
        title: "تم انشاء الدرس بنجاح",
        status: "success",
      });
  ...
  return (
    <>
      ...
      <Stack as="form" onSubmit={handleSubmit(onSubmit)} spacing={4}>
        <Heading size="md">انشاء حصة</Heading>
        <Divider />
        ...
        <Controller
          ...
          render={({ field }) => (
            <PDFUploader
              label="رفع ملف"
              pdf={field.value}
              onChangePdf={(file) => field.onChange(file)}
              onRemovePdf={(file) => field.onChange("")}
            />
          )}
        />
        <ErrorText>{errors.pdf?.message}</ErrorText>
