...
export default function Index() {
  ...
  const updateExerciseHelper = useCallback(async (data) => {
    const req = await axiosInstance.put(`/Exercises/${id}`, data, {
      headers: {
        Authorization: `Bearer ${user.data.token}`,
      },
    });
  ...
  const onSubmit = async (data) => {
    try {
      if (data.video instanceof File) {
        ...
        await updateExerciseHelper(DataSend);
      } else {
        const DataSend = {
          ...data,
          bodyPart: data.bodyPart._id,
