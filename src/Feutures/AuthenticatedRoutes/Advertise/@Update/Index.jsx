...
export default function Index() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const req = await axiosInstance.put(`advertise/${id}`, FormFields, {
        headers: {
  ...
  useEffect(() => {
    if (data?.data) {
      reset({
        ...data?.data,
        targetModel: Models.find((item) => {
          return item.value === data?.data?.targetModel;
        }).en,
        targetModelId: data?.data?.targetModelId,
      });
    }
  }, [JSON.stringify(data?.data)]);

  return (
    <Stack as={Skeleton} isLoaded={!loading} w="100%" p="3">
      <Controller
        control={control}
        name="image"
        render={({ field }) => {
