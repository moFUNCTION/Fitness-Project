...
const UpdateToolOrMachineModal = ({ item, onClose, isOpen, onRenderPage }) => {
  ...
  return (
    ...
    >
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>تعديل الاداه او الجهاز</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <InputElement
            errors={errors}
...
export const ToolBox = ({ _id, title, createdAt, updatedAt, onRenderPage }) => {
  const { user } = useAuth();
  const toast = useToast({
    position: "top-right",
    duration: 3000,
  ...
  const HandleDelete = async (data) => {
    try {
      await onDeleteTool({
        url: `/toolOrMachine/${_id}/moveToTrash`,
        headers: {
          Authorization: `Bearer ${user.data.token}`,
        },
        body: data,
